class MijiPanel extends BaseView {
		...
		for (let i = 0; i < 8; i++) {
			...
			if (ErrorLog.Assert(ins.miji, "MijiPanel   data.miji is null")) {
				...
				if (ErrorLog.Assert(numList, "MijiPanel   numList " +
					...
					let isLiang = !numList[i] ? false : (numList[i].id == 0 ? false : true);
	...
	private setOpenDesc(): void {
		...
		if (config) {
			// this.curZsLv.text = `${config.zsLevel}转或者VIP${config.vipLevel}开启第${config.vipLevel}个秘籍格`;
		...
		if (isSuss == 1) {
			...
			let callback = ()=>{
				...
				TimerManager.ins().doTimer(500, 1, () => {
					...
					// UserTips.ins().showTips( "秘籍学习完成" + tempName);
					UserTips.ins().showTips("秘籍镶嵌完成" + tempName);
					//判定是否镶嵌过 没有则设置标识 用于判断秘籍红点新规则
	...
	private onTap(e: egret.TouchEvent): void {
		...
		switch (e.target) {
			case this.getBtn:
				...
				if (isLearn) {
					WarnWin.show(`已镶嵌《${tempName}》,将直接替代《${tempName}》获得《${this.curName}》效果`, () => {
