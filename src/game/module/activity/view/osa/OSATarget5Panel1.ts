 ...
 * 合服7天登录
 */
class OSATarget5Panel1 extends BaseView {
    ...
    private changeList(): void {
        let day: number = 1;
        let dayNum: number = (this._activityData as ActivityType5Data).getCurDay();//Activity.ins().hfLoginDay;
        if(!dayNum){
            dayNum = day;
        }
...
//合服登录item
class ActivityHFRenderItem extends SevenDayItemRender{
    public activityID:number;
    constructor(){
        super();
    }
    protected dataChanged(): void {
        let activityID:number = this.activityID;
        let _activityData = Activity.ins().getActivityDataById(activityID) as ActivityType5Data;
        let index = this.data;
        let config = GlobalConfig['ActivityType5Config'][activityID][(index)+""];
        let currDay =  (_activityData as ActivityType5Data).getCurDay();
        let flag: boolean = ((_activityData.recrod >> config.day & 1) == 1);
        if (flag) {
