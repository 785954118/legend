...
class OSATargetItemRender1 extends BaseItemRender {
		...
		switch (e.target){
			case this.get:
				if( this.isGet ){
					...
					let config:ActivityType2Config | PActivity2Config = this.data;
					if( config ){
	...
	protected dataChanged(): void {
		...
		let act2Data: ActivityType2Data|PActivityType2Data;
		if( this.data instanceof ActivityType2Config ){
			config = this.data;
			act2Data = Activity.ins().getActivityDataById(config.Id) as ActivityType2Data;
		}else if( this.data instanceof PActivity2Config ){
			config = this.data;
			act2Data = PActivity.ins().getActivityDataById(config.Id) as PActivityType2Data;
		}
	...
	//
	private getRedPoint(config: ActivityType2Config|PActivity2Config){
