/**
 * Created by hrz on 2018/1/2.
 */

class OSATarget18Panel1 extends BaseView {
	...
	public init(): void {
		let configs: ActivityType18Config[] = GlobalConfig.ActivityType18Config[this.activityID];
		let config: ActivityType18Config = configs[0];
	...
	private buyHunt(index: number): void {
		...
		this.huntType = index;
		let configs = GlobalConfig.ActivityType18Config[this.activityID];
		let item: ItemData = UserBag.ins().getBagGoodsByTypeAndId(UserBag.BAG_TYPE_OTHTER, configs[index].item);
		if (item && item.count) {
	...
	private huntWarnFun(): void {
		let huntOnce: number = GlobalConfig.ActivityType18Config[this.activityID][this.huntType].yb;
	...
	private updateBox(): void {
		let configs: ActivityType18Config[] = GlobalConfig.ActivityType18Config[this.activityID];
		let boxConfig: ActivityType18Config[] = [];
		...
		let state: number;
		let config: ActivityType18Config;
		let i: number = 0;
	...
	private updateMaterial(): void {
		let item = UserBag.ins().getBagGoodsByTypeAndId(0, GlobalConfig.ActivityType18Config[this.activityID][1].item);
