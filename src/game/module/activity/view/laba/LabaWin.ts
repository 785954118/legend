/**
 * Created by hjh on 2018/1/12.
 */

class LabaWin extends BaseEuiView {
	/** 关闭按钮 */
	...
	public initUI(): void {
		super.initUI();
		this._pageStyle = ActivityPageStyle.LABA;
	...
	private updateView() {
		...
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().getActivityDataById(+k).isOpenActivity()) {
				...
				if (!ErrorLog.Assert(data, "laba  data   " + k))
...
ViewManager.ins().reg(LabaWin, LayerManager.UI_Main);