 ...
 * 龙抬头入口
 */
class LongTaiTouWin extends BaseEuiView {
	/** 关闭按钮 */
	...
	public initUI(): void {
		super.initUI();
		this._pageStyle = ActivityPageStyle.LONGTAITOU;
	...
	public static openCheck(...param: any[]): boolean {
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().activityData[k].pageStyle != ActivityPageStyle.LONGTAITOU)
				continue
	...
	private updateView() {
		...
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().getActivityDataById(+k).isOpenActivity()) {
				...
				if (!ErrorLog.Assert(data, "longtaitou  data   " + k))
		...
		for (let k in this._datas) {
			...
			if (act && act.id == id) {
				...
				if (!hide) {
					let panel: ActivityPanel = ActivityPanel.create(id, ActivityType.Normal);
					if (!panel)
						continue;
					panel.top = 0;
					panel.left = 0;
	...
	private onClickMenu(e: egret.TouchEvent): void {
		...
		if (this.activityPanelList[this.selectIndex].activityID > 10000)
			...
			if (config) {
				this.setOpenIndex(this.selectIndex);
				// if( config.activityType == ActivityDataFactory.ACTIVITY_TYPE_18 ){
...
ViewManager.ins().reg(LongTaiTouWin, LayerManager.UI_Main);