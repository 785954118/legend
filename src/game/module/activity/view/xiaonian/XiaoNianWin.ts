 ...
 * 小年入口
 */
class XiaoNianWin extends BaseEuiView {
	/** 关闭按钮 */
	...
	public initUI(): void {
		super.initUI();
		this._pageStyle = ActivityPageStyle.XIAONIAN;
	...
	public static openCheck(...param: any[]): boolean {
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().activityData[k].pageStyle != ActivityPageStyle.XIAONIAN)
				continue
	...
	private updateView() {
		...
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().getActivityDataById(+k).isOpenActivity()) {
                ...
                if (!ErrorLog.Assert(data, "xiaonian  data   " + k))
...
ViewManager.ins().reg(XiaoNianWin, LayerManager.UI_Main);