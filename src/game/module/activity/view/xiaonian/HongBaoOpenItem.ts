...
class HongBaoOpenItem extends BaseItemRender {
	...
	public inithongbaodata(actid, hongbaoid) {
		this.actid = actid;
	...
	protected dataChanged(): void {
		...
		let config: ActivityType12Config = GlobalConfig.ActivityType12Config[this.data.actId][index];
	...
	public playAni(arr: { job: number, sex: number, name: string, yb: number, gold: number }[], callFun: Function, obj: any) {
		...
		let tw1: egret.Tween = egret.Tween.get(upSr, {
			onChange: () => {
		...
		let tw2: egret.Tween = egret.Tween.get(downSr, {
			onChange: () => {
