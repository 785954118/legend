 ...
 * 开门红入口
 */
class KaiMenWin extends BaseEuiView {
	/** 关闭按钮 */
	...
	public initUI(): void {
		super.initUI();
		this._pageStyle = ActivityPageStyle.KAIMENHONG;
	...
	public static openCheck(...param: any[]): boolean {
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().activityData[k].pageStyle != ActivityPageStyle.KAIMENHONG)
				continue
	...
	private updateView() {
		...
		for (let k in Activity.ins().activityData) {
			...
			if (Activity.ins().getActivityDataById(+k).isOpenActivity()) {
				...
				if (!ErrorLog.Assert(data, "kaimenhong  data   " + k))
...
ViewManager.ins().reg(KaiMenWin, LayerManager.UI_Main);