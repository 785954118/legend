...
class ActivityType2Data extends ActivityBaseData {
	...
	public areabuyReward: number[];

	/** 全服购买次数 */
	public serverBuyData:number[];
	/** 全战区购买次数 */
	public areaBuyData:number[];
	...
	public update(bytes: GameByteArray): void {
		...
		this.areaBuyData[id] = bytes.readInt(); //全战区已购买次数
		this.areabuyReward[id] = bytes.readInt(); //额外奖励领取标记位
	...
	public initBuyData(bytes: GameByteArray){
		...
		for (let i: number = 1; i <= count; i++)//每一个礼包已购买次数
		{
			...
			this.areaBuyData[i] = bytes.readInt(); //全战区已购买次数
			this.areabuyReward[i] = bytes.readInt(); //额外奖励领取标记位
		}
		//个人和全战区取最大值
		for( let j = 0; j <= count;j++ ){
			this.areaBuyData[j] = Math.max(this.areaBuyData[j],this.buyData[j]);
	 ...
	 * 领取战区奖励状态
	 * index 活动序号 1开始
	 * zqindex countReward奖励序号 1开始
	 * */
	public getAreaReward(index:number,zqindex:number){
		let config = GlobalConfig.ActivityType2Config[this.id][index];
