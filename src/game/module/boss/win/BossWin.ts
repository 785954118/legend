...
class BossWin extends BaseEuiView {
	...
	public initUI(): void {
		...
		let redPointGap:number = 112;//红点间的间隔
	...
	public open(...param: any[]): void {
		...
		this.observe(UserFb.ins().postUpDataInfo, this.updatePanel);//特权扫荡
		this.observe(UserFb.ins().postUpDataInfo, this.updateRedPoint);//特权扫荡
				...
				if (e.target instanceof eui.Button) {
					...
					switch (e.target.name) {
						...
						case "publicChallenge":
							...
							// 	UserTips.ins().showTips("挑战次数不足，首冲即可参与BOSS之家，无限刷就是爽！");
							// }
