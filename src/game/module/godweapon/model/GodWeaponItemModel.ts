...
class GodweaponItemModel extends BaseClass {
	...
	public init(): void {
		...
		for (let i in GlobalConfig.GodweaponItemConfig) {
			...
			if (quality < 4) {
				let ary: number[] = this._qualityItemMap[quality];
				if (ary == undefined) {
					ary = [];
				}
				ary.push(cfg.id);
				...
				if (!cfg.skill || cfg.skill.length <= 1) {
					...
					let dataAry: number[] = this._jobItemMap[job];
	...
	public isCanCompound(): boolean {
		...
		let qualityItemMap = this.qualityItemMap;
		for (let i in qualityItemMap) {
	...
	public getFuseItemList(): any {
		let list = {};
		let canFuse = false;
		let jobItemMap = this.jobItemMap;
		for (let i in jobItemMap) {
	...
	public getJobFuseItemList(itemList: number[]): GodweaponItem[] {
		...
		for (let i = 0; i < count; i++) {
			...
			let itemCount = UserBag.ins().getBagGoodsCountById(0, itemList[i]);
			if (itemCount > 0) {
