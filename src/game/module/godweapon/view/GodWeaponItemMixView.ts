...
class GodweaponItemMixView extends BaseView {
	...
	private PAGE_SIZE = 5;
	private materialTxt:eui.Label;
	private materialTxtImg:eui.Label;
	private mixMaterialImg:eui.Image;
	private mixMaterial:eui.Group;
	private descTxt:eui.Label;
	private isMaterial:boolean;//是否添加的圣物精华
	private MaterialItemId:number;
	private mixlabel:eui.Label;
	public open(): void {
		...
		switch (e.currentTarget){
			case this.mixMaterial:
				if( this.isMaterial ){
					...
					let itemData = UserBag.ins().getBagItemById(this.MaterialItemId);
					if( itemData ){
						...
						// UserTips.ins().showTips(`没有可添加的道具`);
					}
				}
				break;
		}
	}
	private updateShengWu(){
	...
	private playTween(){
		...
		let alpha = this.descTxt.alpha?0:1;
