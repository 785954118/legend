...
class RuneTipsWin extends BaseEuiView {
	...
	private onUser(evt: egret.TouchEvent):void{
		let conf = Rune.ins().getRuneMergeByID(this.item_config_id);
		if(conf){
	...
	private setData(id: number): void {
		...
		if (itemLv == 0) {
			if (!this.cur_desc2) {//主要指非战纹槽
				lvstr = " lv." + itemLv;
			}
		} else {
			if (ItemConfig.getSubType(itemCfg) != 0) {//主要指战纹精华 //itemCfg.type != 1 &&
				lvstr = " lv." + itemLv;
			}
		}
