...
class HeartMethodTips extends BaseEuiView {
	...
	private updateUI(){
		...
		this.nameLabel.textFlow = TextFlowMaker.generateTextFlow1(`|C:${ItemConfig.getQualityColor(itemConfig)}&T:${itemConfig.name}`);
			...
			//战力
		let power = UserBag.getAttrPower(config.attr);
		this.powerPanel.setPower(power);

		//属性描述
		let strAttrName = "";
		let strAttrValue = "";
		for( let i = 0;i < HeartMethod.ins().proShowList.length;i++ ){
			for ( let j = 0; j < config.attr.length;j++ ){
				if( HeartMethod.ins().proShowList[i].id == config.attr[j].type ){
					let sname = HeartMethod.ins().getAttrStrByType(config.attr[j].type);
					if( !sname )continue;
					if( !config.attr[j].value )continue;//属性值0跳过
					strAttrName += sname;
