...
class SamsaraModel extends BaseClass {
	...
	constructor() {
		...
		this.samsaraDescAry = [data.hellsName, data.ghostsName, data.animalsName, data.demigodsName, data.humansName, data.godsName];
	...
	public getEquipSamsaraLv(equipId: number): number {
		let zsLv = GlobalConfig.ItemConfig[equipId].zsLevel;
		return zsLv;
	}
	 ...
	 * @param roleIndex
	 */
	public getExtraPower(role: Role, addPercent: number, attrType: number): number {
		let power = 0;
		let attr = 0;
		for (let i = EquipPos.HAT; i <= EquipPos.SHIELD; i++) {
			let data: EquipsData = role.getEquipByIndex(i);
			if (data.item.itemConfig) {
				let itemId = data.item.itemConfig.id;
				if (itemId && GlobalConfig.EquipConfig[itemId].baseAttr1) {
					let attr1 = GlobalConfig.EquipConfig[itemId].baseAttr1;
					attr += attr1.value;
				}
			}
		}
		if (attrType) {
			...
			let config = powerConfig[attrType];
	...
	public composeMaxEquip(itemId: number): number {
		let num = UserBag.ins().getBagGoodsCountById(UserBag.BAG_TYPE_EQUIP, itemId);
		if (num) {
	...
	public composeEquip(material: RewardData): RewardData {
		...
		let num: number = material.count + UserBag.ins().getBagGoodsCountById(UserBag.BAG_TYPE_EQUIP, itemId) + this.wearCount(itemId);
		let data;
