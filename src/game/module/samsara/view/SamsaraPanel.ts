...
class SamsaraPanel extends BaseView {
	...
	public updateView(): void {
		...
		// attrDesc += `\n${AttributeData.getAttrStrByType(AttributeType.attTogetherHolyDamege)} : ${this.getValue(cfg.attrs, AttributeType.attTogetherHolyDamege)}`;
		this.attr1.text = attrDesc;
		let power = UserBag.getAttrPower(cfg.attrs) * 3;
		this.powerPanel1.setPower(power);
		let percent;

		let nextLv = lv + 1;
		let isMax = nextLv >= CommonUtils.getObjectLength(GlobalConfig.ReincarnationLevel);
		if (isMax) {
			...
			let nextPower = UserBag.getAttrPower(nextCfg.attrs) * 3;
	...
	private setMaxSamsaraState(isMax: boolean): void
	{
