...
class HeirloomDownView extends BaseEuiView {
	...
	public getHeirloomSource(){
		...
		let itemData:ItemData[] = UserBag.ins().getBagEquipsByQuality(5,UserBag.BAG_DESC_TYPE_HEIR,UserBag.BAG_TYPE_OTHTER);
	...
	public getHeirloomGoSource(){
		...
		// let item:ItemData = UserBag.ins().getBagGoodsByDescIndex(246);//传世宝钻
		let item:ItemData = UserBag.ins().getBagItemById(200141);//传世宝钻
