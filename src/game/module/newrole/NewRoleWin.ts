...
class NewRoleWin extends BaseEuiView {
	...
	private roleBody: string[][] = [
		[/**男*/ "op_zs0_png", /**女*/"op_zs1_png"], //战士
		[/**男*/ "op_fs0_png", /**女*/"op_fs1_png"], //法师
		[/**男*/ "op_ds0_png", /**女*/"op_ds1_png"]];//道士
	private roleWeapon: string[] = [
		"", //战士
		"", //法师
		""];//道士
	private rolePos: number[][] = [
		[100, 0, 1, 3],//x,y,scale,layer
		[-4, 79, 0.7, 2],
		[336, 79, 0.7, 2]
	];
		...
		switch (e.target) {
			case this.openBtn:
				if (!this.canOpen) {
					...
					let openIndex: number = SubRoles.ins().subRolesLen;
					let config: NewRoleConfig = GlobalConfig.NewRoleConfig[openIndex];
					let win = WarnWin.show(`VIP等级不足，VIP${config.vip}可以提前开启第${openIndex+1}角色，是否前往开启？`, null,null,
						() => {
