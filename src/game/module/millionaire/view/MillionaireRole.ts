...
class MillionaireRole extends egret.DisplayObjectContainer{
	...
	private model3:MovieClip;//传世
	private prefix3:string;//传世模型前缀

	private model4:MovieClip;//兵魂
	private prefix4:string;//兵魂模型前缀
	...
	private initModel(){
		...
		//层级关系
		this.weightLayers = [];
		this.weightLayers[MillionaireView.DIR_UP]    = CharEffect.FRAME_ODER[0];//上
		this.weightLayers[MillionaireView.DIR_RIGHT] = CharEffect.FRAME_ODER[2];//右
		this.weightLayers[MillionaireView.DIR_LEFT]  = CharEffect.FRAME_ODER[6];//左
		this.weightLayers[MillionaireView.DIR_DOWN]  = CharEffect.FRAME_ODER[4];//下
	...
	public updateModel(){
		if( this.preName != this.stateName ){
			...
			if( this.prefix3 )
				this.model3.playFile(RES_DIR_EFF + this.prefix3,-1);//传世
			if( this.prefix4 )
				this.model4.playFile(RES_DIR_WEAPON + this.prefix4 + "_" + this.stateName,-1);//兵魂
			this.setMovieFromWeight();
		}
	}
