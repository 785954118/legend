 ...
 */
class WeaponsData {
	private WeaponsInfoData:Map<Map<WeaponsInfo>>;//兵魂部位 索引0没数据
	private WeaponsSoulInfoData:Map<WeaponsSoulInfo>;//兵魂 索引0没数据
	...
	/**
	 * 获取当前兵魂套装等级(取激活后 各个部位最小等级 若有一个部件没激活返回null)
	 * @param 兵魂id
	 * */
		...
		if( id > 0 ){
			...
			if( role ){
				...
				if( eff ){
					...
					Activationtongyong.show(0, this.WeaponsSoulInfoData[id].name, `new_duanzao_wepon1`,true,()=>{
