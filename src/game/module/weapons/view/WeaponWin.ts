...
class WeaponWin extends BaseEuiView {
	...
	private desc1: eui.Label;//文字:使用兵魂
	private desc2: eui.Label;//文字:激活技能

	private powerPanel: PowerPanel;

	public roleId: number;
	public weaponId: number;//选中的兵魂
	...
	private updateBtn() {
		// let flexibleCount = SubRoles.ins().getSubRoleByIndex(this.roleId).weapons.flexibleCount;
		this.turnBtn.visible = true;//flexibleCount < GlobalConfig.WeaponSoulBaseConfig.maxItemNum;
	}

	public init(selectId: number) {
		this.soulArr = [];
		this.listdata = [];
		...
		for (let k in GlobalConfig.WeaponSoulConfig) {
			...
			let tmp: { isuse: boolean, id: number, isRedPoint: boolean, level: number, isSelect: boolean, showId: number, job: number } = {
				...
				id: wsinfo && wsinfo.id && wss ? wss.id : 0,//用作判断是否激活
	...
	private updateUI() {
		...
		let attrValue = [0, 0, 0, 0];//攻击 生命 魔防 物防
		let isHave = false;//是否有部位被激活(激活意味着有属性 用于判断是否让兵魂之灵丹药显示生效)
		...
		//兵魂之灵
		let index: number = role.weapons.flexibleCount ? role.weapons.flexibleCount - 1 : 0;
		let cfg: WeaponSoulItemAttr = GlobalConfig.WeaponSoulItemAttr[index];
		if (isHave && cfg) {
			for (let i = 0; i < cfg.attr.length; i++) {
