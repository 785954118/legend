...
class WeaponPanel extends BaseEuiView {
	...
	private ringname:eui.Label;//兵魂名字
	private levellabel:eui.Label;//兵魂等级
	...
	private updateUI(id:number){
		...
		//兵魂属性描述
		this.ringname.text = wsconfig.name;//兵魂名字
		...
		else if( role.weapons.weaponsId == ws.id ){//选中的兵魂是正在使用的兵魂
			this.upGradeBtn0.label = "取 消";
			this.huanhuaImage.visible = true;
			this.redPoint.visible = false;
		}
		else if( !role.weapons.weaponsId ){//目前没有使用任何兵魂
