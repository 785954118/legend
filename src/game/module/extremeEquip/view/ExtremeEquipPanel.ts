...
class ExtremeEquipPanel extends BaseView {
	...
	public selectEquip(subType: number): void {
		...
		if ( !zhiZunLv ) {//未激活
			...
			let power2 = Math.floor(UserBag.getAttrPower(preCfg.attrs)) + preCfg.ex_power;
			power2 += this.getChainAddPower(subType,zhiZunLv);
			this.powerPanel2.setPower(power2);
			let itemCount = UserBag.ins().getBagGoodsCountById(UserBag.BAG_TYPE_OTHTER, preCfg.materialInfo.id);
			...
			let power2 = Math.floor(UserBag.getAttrPower(cfg.attrs)) + cfg.ex_power;
			power2 += this.getChainAddPower(subType,zhiZunLv);
			this.powerPanel2.setPower(power2);
		} else {
			...
			this.cost.text = item.name;
			let itemCount = UserBag.ins().getBagGoodsCountById(UserBag.BAG_TYPE_OTHTER, cfg.materialInfo.id);
			...
			let power0 = Math.floor(UserBag.getAttrPower(preCfg.attrs)) + preCfg.ex_power;
			let power1 = Math.floor(UserBag.getAttrPower(cfg.attrs)) + cfg.ex_power;
