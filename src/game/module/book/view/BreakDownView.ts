...
class BreakDownView extends BaseEuiView {
	...
	public static _fitleEquip:Map<SpecialEquipsConfig>;
	public constructor() {
		super();
	...
	private getLegendSource() {
		...
		//神装分解规则
		if( this._ext == 4 ){
			let totalItemData = UserBag.ins().getBagEquipsByQuality(4);
			...
			//传奇分解规则(红点规则不变 但所有热血装备都出现在分解列表)
			itemData = UserBag.ins().getBagEquipsByQuality(5);// UserBag.ins().getLegendOutEquips();
		}
	...
	private getBookSource() {
		let itemData = UserBag.ins().getBagGoodsByType(this._ext);
		itemData.sort((n1, n2) => {
